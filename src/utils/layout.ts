...
export function applyMindmapLayout(graph: Graph, direction: MindmapLayoutDirection, startNode?: Node) {
  ...
  layoutRoots.forEach(root => {
    if (direction === 'radial') {
      ...
      applyTreeLayout(graph, layoutDir, root);
...
export function applyTreeLayout(graph: Graph, direction: LayoutDirection = 'LR', startNode?: Node) {
  const nodes = graph.getNodes();
  if (nodes.length === 0) return;
...
function applyBalancedLayout(graph: Graph, root: Node) {
...
function applyRadialLayout(graph: Graph, root: Node) {
  const tree = buildTree(graph, root);
  const rootPos = root.getPosition();
  const rootCenter = { 
...
function positionRadialNodes(node: TreeNode, depth: number, cx: number, cy: number) {
  const angle = typeof node._angle === 'number' ? node._angle : 0;
  const RADIUS_GAP = 220;
  ...
  node.children.forEach((child) => positionRadialNodes(child, depth + 1, cx, cy));
}
...
// ============ Timeline Layout ============
export function applyTimelineLayout(graph: Graph, orientation: 'horizontal' | 'vertical' = 'horizontal') {
  const nodes = graph.getNodes();
  if (nodes.length === 0) return;

  // Sort nodes by data.date if available, otherwise by position
